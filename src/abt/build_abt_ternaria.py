...
COLUMN_ALIASES = {
    ...
    "media_enem": ["media_enem", "media_nota_enem"],
    "nota_corte_gp": ["nota_corte_gp", "nota_corte_grupo_preferencia"],
    "idade": ["idade", "idade_inscrito", "idade_candidato"],
    "data_nascimento": ["data_nascimento", "dt_nascimento", "nascimento"],
    "nome_curso": ["nome_curso", "no_curso", "curso", "nome_do_curso"],
    "ano": ["ano", "ano_processo_seletivo"],
    "semestre": ["semestre", "semestre_processo_seletivo"],
    "opcao_curso": ["opcao_curso", "opcoes_cursos_inscricao"],
    "conceito_curso_gp": ["conceito_curso_gp", "conceito_curso", "conceito"],
    "turno": ["turno"],
    "ensino_medio_escola_publica": ["ensino_medio_escola_publica"],
    "regiao_ies_alvo": ["regiao_ies_alvo", "regiao_ies"],
    "natureza_juridica_mantenedora": ["natureza_juridica_mantenedora"],
    "etnia_cor": ["etnia_cor", "raca_cor", "cor_raca", "cor_ou_raca"],
    "sexo": ["sexo"],
    "regiao_morar": ["regiao_morar", "regiao_residencia"],
    "organizacao_academica": ["organizacao_academica", "organizacao_academica_ies"],
    "subarea_conhecimento": ["subarea_conhecimento"],
    "concluiu_curso_superior": ["concluiu_curso_superior", "conceito_concluiu_curso_superior"],
    "beneficiado_creduc_fies": ["beneficiado_creduc_fies"],
    "uf_local_oferta": ["uf_local_oferta", "sg_uf_local_oferta"],
    "id_estudante": ["id_estudante", "cpf", "cpf_hash", "identificador_estudante"],
}
...
def log(message: str) -> None:
    ...
    path = LOGS_DIR / "abt_logit_ternario.log"
...
def aplicar_recorte_medicina(df: pd.DataFrame) -> pd.DataFrame:
    coluna_subarea = encontrar_coluna(df, "subarea_conhecimento")
    coluna_nome_curso = encontrar_coluna(df, "nome_curso")

    if coluna_subarea is not None:
        subarea_norm = (
            df[coluna_subarea]
            .astype("string")
            .fillna("")
        ...
        mascara = subarea_norm.eq("MEDICINA")

        if mascara.sum() > 0:
            out = df[mascara].copy()
            log(f"[OK] Recorte Medicina aplicado por subarea_conhecimento | linhas: {len(out)}")
            return out

    if coluna_nome_curso is None:
        raise ValueError("Não encontrei subarea_conhecimento nem nome_curso para fazer o recorte de Medicina.")

    nome_norm = (
        df[coluna_nome_curso]
        .astype("string")
        .fillna("")
        .map(lambda x: remover_acentos(x).upper().strip())
    )
    ...
    out = df[mascara].copy()
    log(f"[OK] Recorte Medicina aplicado por nome_curso | linhas: {len(out)}")

    return out


def aplicar_recorte(df: pd.DataFrame, recorte: str) -> pd.DataFrame:
    if recorte == "geral":
        log(f"[OK] Recorte geral aplicado | linhas: {len(df)}")
        return df.copy()

    if recorte == "medicina":
        return aplicar_recorte_medicina(df)
...
def construir_colunas_canonicas(df_base: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    for nome in COLUNAS_CANONICAS:
        if nome in {"gap", "renda_gap", "target_ternario"}:
            continue
...
def path_saida_abt(recorte: str) -> Path:
    if recorte == "geral":
...
def salvar_abt(recorte: str, abt: pd.DataFrame, metadata: dict) -> None:
    ...
    resumo_path = LOGS_DIR / f"abt_logit_ternario_{recorte}_resumo.csv"
...
def run(recorte: str = "geral") -> None:
    log("=" * 80)
    log(f"BUILD ABT LOGIT TERNÁRIO | {recorte.upper()}")
    log("=" * 80)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Constrói ABT ternária para regressão logística multinomial.")
