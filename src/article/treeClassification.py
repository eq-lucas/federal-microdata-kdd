...
from src.modeling.treeClassification_utils import (
    EXPERIMENTS,
    FEATURE_BLOCKS,
    TEST_SIZE,
    RANDOM_STATE,
...
def target_suffix(target: str, recorte: str) -> str:
    ...
    if target == "binario":
        return f"treeClassification_binario_recorte_{recorte}"
    return f"treeClassification_ternario_recorte_{recorte}"
...
def tables_tree_dir(target: str, recorte: str) -> Path:
    return TABLES_DIR / f"secao_treeClassification_{normalizar_target(target)}_recorte_{normalizar_recorte(recorte)}"
...
def log(message: str) -> None:
    ...
    path = LOGS_DIR / "article_treeClassification.log"
...
def carregar_modelo_metadata_abt(target: str, recorte: str, avaliacao: str, experimento: str):
    ...
    model_path = models_dir(target, recorte, avaliacao) / f"{cfg['model_dir_name']}_{experimento.lower()}.joblib"
    model_meta_path = models_dir(target, recorte, avaliacao) / f"{cfg['model_dir_name']}_{experimento.lower()}_metadata.json"
...
def carregar_importancias(target: str, recorte: str, avaliacao: str, experimento: str) -> pd.DataFrame:
    ...
    recorte = normalizar_recorte(recorte)
    cfg = target_config(target)
    path = diagnostics_dir(target, recorte, avaliacao) / f"{cfg['model_dir_name']}_importancias_agregadas.csv"
    if not path.exists():
...
def salvar_dados_probabilidades(df_pred: pd.DataFrame, target: str, recorte: str, avaliacao: str, experimento: str) -> dict:
    ...
    nome = f"dados_probabilidades_treeClassification_{target}_recorte_{recorte}_{avaliacao}_{experimento.lower()}"
...
def gerar_figura_arvore(modelo, target: str, recorte: str, avaliacao: str, experimento: str) -> dict:
    nome = f"figura_arvore_treeClassification_{normalizar_target(target)}_recorte_{normalizar_recorte(recorte)}_{avaliacao}_{experimento.lower()}"
    cfg = target_config(target)
...
def run(target: str = "binario", recorte: str = "geral", avaliacao: str = "in_sample", experimento: str = "E5") -> None:
    ...
    log("=" * 80)
    log(f"ARTICLE: TREECLASSIFICATION | {target.upper()} | {recorte.upper()} | {avaliacao.upper()} | {experimento}")
    log("=" * 80)
    ...
    resumo_path = LOGS_DIR / f"article_treeClassification_{target}_recorte_{recorte}_{avaliacao}_{experimento.lower()}_resumo.csv"
