...
from src.modeling.logit_binario_utils import diagnostics_dir
...
def log(message: str) -> None:
    ...
    path = LOGS_DIR / "article_apendice_logit_binario.log"
    ...
    tabela = pd.DataFrame(
        {
            ...
            "ROC-AUC": df["roc_auc"].map(fmt_float),
            "Acurácia balanceada": df["balanced_accuracy"].map(fmt_float),
            "F1": df["f1"].map(fmt_float),
            "Renda": df["coef_renda_per_capita"].map(fmt_float),
            "Desempenho (gap)": df["coef_gap"].map(fmt_float),
            "Interação": df["coef_renda_gap"].map(fmt_float),
    ...
    tabela = pd.DataFrame(
        {
            ...
            "ROC-AUC": df["roc_auc"].map(fmt_float),
            "Renda": df["coef_renda_per_capita"].map(fmt_float),
            "Desempenho": df["coef_gap"].map(fmt_float),
            "Interação": df["coef_renda_gap"].map(fmt_float),
...
def gerar_tabela_completa(recorte: str, avaliacao: str) -> dict:
    ...
    out_dir = APPENDIX_LOGIT_DIR / avaliacao / recorte
    out_dir.mkdir(parents=True, exist_ok=True)

    nome = f"tabela_logit_binario_{recorte}_{avaliacao}"
    base = out_dir / nome
    ...
    out.update(
        salvar_tabela_latex(
            tabela,
            base,
            caption=f"Comparação de experimentos da regressão logística binária, recorte {recorte}, avaliação {avaliacao}.",
            label=f"tab:{nome}",
        )
    )
...
def gerar_tabela_compacta(recorte: str, avaliacao: str) -> dict:
    ...
    nome = f"tabela_compacta_logit_binario_{recorte}_{avaliacao}"
    base = out_dir / nome
    ...
    out.update(
        salvar_tabela_latex(
            tabela,
            base,
            caption=f"Resumo compacto das especificações da regressão logística binária, recorte {recorte}, avaliação {avaliacao}.",
            label=f"tab:{nome}",
        )
    )
    out.update(
...
def gerar_matriz_confusao(recorte: str, avaliacao: str, experimento: str) -> dict:
    df = carregar_metricas(recorte, avaliacao)
...
def run(recorte: str = "geral", avaliacao: str = "all", experimento: str = "E5") -> None:
    ...
    resumo_path = LOGS_DIR / "article_apendice_logit_binario_resumo.csv"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Gera tabelas e matriz de confusão do apêndice da regressão logística binária.")
