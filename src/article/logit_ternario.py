...
from src.modeling.logit_ternario_utils import (
...
def figures_logit_dir(recorte: str) -> Path:
    recorte = normalizar_recorte(recorte)

    if recorte == "medicina":
        return FIGURES_DIR / "logit_ternario_recorte_medicina"

    return FIGURES_DIR / "logit_ternario_recorte_geral"


def tables_logit_dir(recorte: str) -> Path:
    recorte = normalizar_recorte(recorte)

    if recorte == "medicina":
        return TABLES_DIR / "secao_4_5_logit_ternario_recorte_medicina"

    return TABLES_DIR / "secao_4_5_logit_ternario_recorte_geral"
...
def log(message: str) -> None:
    ...
    path = LOGS_DIR / "article_logit_ternario.log"
...
def carregar_modelo_metadata_abt(recorte: str, avaliacao: str, experimento: str):
    ...
    model_path = models_dir(recorte, avaliacao) / f"logit_ternario_{experimento.lower()}.joblib"
    model_meta_path = models_dir(recorte, avaliacao) / f"logit_ternario_{experimento.lower()}_metadata.json"
...
def selecionar_base_para_probabilidades(abt: pd.DataFrame, avaliacao: str) -> tuple[pd.DataFrame, str]:
    ...
    if avaliacao == "holdout_80_20":
        if "target_ternario" not in abt.columns:
            raise ValueError("A ABT não possui target_ternario para reconstruir o holdout.")

        y = pd.to_numeric(abt["target_ternario"], errors="raise").astype(int)
...
def anexar_probabilidades(modelo, metadata: dict, abt: pd.DataFrame, avaliacao: str) -> pd.DataFrame:
    ...
    indice_contratada = classes.index(2)

    prob = modelo.predict_proba(X)[:, indice_contratada] * 100

    df_pred = abt_prob[["renda_per_capita", "gap", "target_ternario"]].copy()
...
def montar_tabela_principal(metricas: pd.DataFrame) -> pd.DataFrame:
    ...
    tabela = pd.DataFrame(
        [
            ...
            {"Medida": "Coeficiente padronizado: renda, classe contratada", "Valor": fmt_float(row["coef_renda_per_capita_classe_contratada"])},
            {"Medida": "Coeficiente padronizado: desempenho, classe contratada", "Valor": fmt_float(row["coef_gap_classe_contratada"])},
            {"Medida": "Coeficiente padronizado: interação, classe contratada", "Valor": fmt_float(row["coef_renda_gap_classe_contratada"])},
...
def salvar_tabela_principal(metricas: pd.DataFrame, recorte: str, avaliacao: str, experimento: str) -> dict:
    ...
    nome = f"tabela_logit_ternario_recorte_{recorte}_principal_{avaliacao}_{experimento.lower()}"
    base = tables_logit_dir(recorte) / nome
    ...
    out.update(
        salvar_tabela_latex(
            tabela,
            base,
            caption=f"Resumo do modelo logístico multinomial, recorte {recorte}.",
            label=f"tab:{nome}",
        )
    )
    out.update(
...
def salvar_dados_probabilidades(df_pred: pd.DataFrame, recorte: str, avaliacao: str, experimento: str) -> dict:
    recorte = normalizar_recorte(recorte)
    out_dir = tables_logit_dir(recorte)
    out_dir.mkdir(parents=True, exist_ok=True)

    nome = f"dados_probabilidades_logit_ternario_recorte_{recorte}_{avaliacao}_{experimento.lower()}"
...
def gerar_figuras_probabilidades(df_pred: pd.DataFrame, recorte: str, avaliacao: str, experimento: str) -> list[dict]:
    ...
    nome_base = f"logit_ternario_recorte_{recorte}_{avaliacao}_{experimento.lower()}"
    ...
    registros.append(
        {
            ...
            **gerar_heatmap_probabilidade(
                df_pred,
                "prob_contratacao",
                figures_logit_dir(recorte) / f"figura_probabilidade_prevista_contratacao_{nome_base}",
            ),
        }
    )

    appendix_fig_dir = appendix_logit_dir(recorte) / avaliacao / recorte / "figuras_curvas"
    registros.append(
        {
...
def run(recorte: str = "geral", avaliacao: str = "in_sample", experimento: str = "E5") -> None:
    ...
    log("=" * 80)
    log(f"ARTICLE: LOGIT TERNÁRIO | {recorte.upper()} | {avaliacao.upper()} | {experimento}")
    log("=" * 80)
    ...
    resumo_path = LOGS_DIR / f"article_logit_ternario_recorte_{recorte}_{avaliacao}_{experimento.lower()}_resumo.csv"
    pd.DataFrame(registros).to_csv(resumo_path, index=False, encoding="utf-8")

    log(f"[OK] Saídas do corpo do artigo: {figures_logit_dir(recorte)} e {tables_logit_dir(recorte)}")
    log(f"[OK] Saídas de apêndice: {appendix_logit_dir(recorte) / avaliacao / recorte}")
    log(f"[OK] Resumo salvo em: {resumo_path}")


def parse_args():
    parser = argparse.ArgumentParser(description="Gera produtos do corpo do artigo para logit ternário.")
