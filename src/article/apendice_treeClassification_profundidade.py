...
from src.modeling.treeClassification_profundidade_utils import diagnostics_dir, normalizar_recorte, normalizar_target, target_config, profile_prefix, validar_profundidade
...
def appendix_tree_dir(target: str, recorte: str) -> Path:
    ...
    return APPENDIX_DIR / "apendice_modelagem" / f"{profile_prefix(CURRENT_PROFUNDIDADE)}_{target}_recorte_{recorte}"


def log(message: str) -> None:
    LOGS_DIR.mkdir(parents=True, exist_ok=True)
    path = LOGS_DIR / f"article_apendice_{profile_prefix(CURRENT_PROFUNDIDADE)}.log"
...
def gerar_tabela_completa(target: str, recorte: str, avaliacao: str) -> dict:
    ...
    nome = f"tabela_{profile_prefix(CURRENT_PROFUNDIDADE)}_{target}_recorte_{recorte}_{avaliacao}"
    base = out_dir / nome
...
def gerar_tabela_compacta(target: str, recorte: str, avaliacao: str) -> dict:
    ...
    out_dir = appendix_tree_dir(target, recorte) / avaliacao / recorte
    nome = f"tabela_compacta_{profile_prefix(CURRENT_PROFUNDIDADE)}_{target}_recorte_{recorte}_{avaliacao}"
    base = out_dir / nome
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_path = base.with_suffix(".csv")
    tabela.to_csv(csv_path, index=False, encoding="utf-8")
...
def gerar_matriz_confusao(target: str, recorte: str, avaliacao: str, experimento: str = "E5") -> dict:
    ...
    nome = f"matriz_confusao_{profile_prefix(CURRENT_PROFUNDIDADE)}_{target}_recorte_{recorte}_{avaliacao}_{experimento.lower()}"
    base = out_dir / nome
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_path = base.with_suffix(".csv")
...
def run(target: str = "binario", recorte: str = "geral", avaliacao: str = "all", experimento: str = "E5", profundidade: int | str = 10) -> None:
    ...
    resumo_path = LOGS_DIR / f"article_apendice_{profile_prefix(CURRENT_PROFUNDIDADE)}_resumo.csv"
