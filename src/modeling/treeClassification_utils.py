...
def target_config(target: str) -> dict:
    return TARGET_CONFIG[normalizar_target(target)]
...
def models_dir(target: str, recorte: str, avaliacao: str) -> Path:
...
def diagnostics_dir(target: str, recorte: str, avaliacao: str) -> Path:
    cfg = target_config(target)
    recorte = normalizar_recorte(recorte)
...
def min_leaf_dinamico(n_train: int, recorte: str) -> int:
    recorte = normalizar_recorte(recorte)
    base_min = 100 if recorte == "medicina" else 250
    return int(max(base_min, round(n_train * 0.0005)))


def criar_modelo(n_train: int, recorte: str) -> DecisionTreeClassifier:
    leaf = min_leaf_dinamico(n_train, recorte)
    return DecisionTreeClassifier(
        criterion="gini",
        splitter="best",
        max_depth=6,
        min_samples_leaf=leaf,
        min_samples_split=max(2 * leaf, 2),
        class_weight="balanced",
        random_state=RANDOM_STATE,
    )


def criar_pipeline(numericas: list[str], categoricas: list[str], n_train: int, recorte: str) -> Pipeline:
    return Pipeline(
        steps=[
            ("preprocessador", criar_preprocessador(numericas, categoricas)),
            ("modelo", criar_modelo(n_train=n_train, recorte=recorte)),
        ]
    )
...
def ajustar_um_experimento(df: pd.DataFrame, metadata_abt: dict, target: str, recorte: str, avaliacao: str, experimento: dict, force: bool) -> tuple[dict, pd.DataFrame, pd.DataFrame]:
    target = normalizar_target(target)
    recorte = normalizar_recorte(recorte)
    cfg = target_config(target)
    ...
    model_dir = models_dir(target, recorte, avaliacao)
    ...
    if model_path.exists() and not force:
        ...
        modelo = criar_pipeline(numericas=numericas, categoricas=categoricas, n_train=len(X_train), recorte=recorte)
        log(
            f"[INÍCIO] {target} | {recorte} | {avaliacao} | {experimento['id']} | "
...
def salvar_resultados(registros: list[dict], importancias_transformadas: list[pd.DataFrame], importancias_agregadas: list[pd.DataFrame], target: str, recorte: str, avaliacao: str) -> None:
    target = normalizar_target(target)
    recorte = normalizar_recorte(recorte)
    cfg = target_config(target)
    out_dir = diagnostics_dir(target, recorte, avaliacao)
...
def run_modelagem(target: str = "binario", recorte: str = "geral", avaliacao: str = "in_sample", force: bool = False) -> None:
    target = normalizar_target(target)
    recorte = normalizar_recorte(recorte)
    ...
    salvar_resultados(registros, importancias_transformadas, importancias_agregadas, target=target, recorte=recorte, avaliacao=avaliacao)
