...
ABT_TERNARIA_GERAL_PATH = getattr(
    C,
    "ABT_TERNARIA_GERAL_PATH",
    ABT_DIR / "abt_contratacao_ternaria_recorte_geral.parquet",
)

ABT_TERNARIA_MEDICINA_PATH = getattr(
    C,
    "ABT_TERNARIA_MEDICINA_PATH",
    ABT_DIR / "abt_contratacao_ternaria_recorte_medicina.parquet",
)


RANDOM_STATE = 42
TEST_SIZE = 0.20
CLASSES_TERNARIAS = [0, 1, 2]
CLASSE_CONTRATADA = 2
...
def log(message: str, log_file: str = "logit_ternario_experimentos.log") -> None:
...
def models_dir(recorte: str, avaliacao: str) -> Path:
    ...
    if recorte == "geral":
        return MODELS_GENERAL_DIR / "logit_ternario" / avaliacao

    if recorte == "medicina":
        return MODELS_MEDICINA_DIR / "logit_ternario" / avaliacao
...
def diagnostics_dir(recorte: str, avaliacao: str) -> Path:
    ...
    return DIAGNOSTICS_DIR / "modeling" / "logit_ternario" / avaliacao / recorte
...
def carregar_abt(recorte: str) -> tuple[pd.DataFrame, dict]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"Rode primeiro: python3 main.py abt ternaria --recorte {recorte}"
        )
...
def preparar_xy(df: pd.DataFrame, blocos: list[str]) -> tuple[pd.DataFrame, pd.Series]:
    if "target_ternario" not in df.columns:
        raise ValueError("A ABT não possui target_ternario.")
    ...
    y = pd.to_numeric(df["target_ternario"], errors="raise").astype(int).copy()
...
def calcular_roc_auc(y_true, proba, classes) -> float:
    try:
        return float(
            roc_auc_score(
                y_true,
    ...
    for i, classe in enumerate(classes):
        partes.append(
            pd.DataFrame(
                {
                    "classe": int(classe),
                    "classe_label": CLASS_LABELS.get(int(classe), str(classe)),
                    "feature_transformada": feature_names,
                    "coeficiente_padronizado": coefs[i],
                    "abs_coeficiente": np.abs(coefs[i]),
                }
            )
        )
...
def coeficiente_exato(coeficientes: pd.DataFrame, feature_name: str, classe: int = CLASSE_CONTRATADA):
    linha = coeficientes[
        (coeficientes["classe"] == classe)
        & (coeficientes["feature_transformada"] == f"num__{feature_name}")
    ]
...
def ajustar_um_experimento(
    ...
    if classes_encontradas != CLASSES_TERNARIAS:
        raise ValueError(
            "O target ternário precisa ter as três classes 0, 1 e 2. "
    ...
    model_path = model_dir / f"logit_ternario_{experimento['id'].lower()}.joblib"
    model_meta_path = model_dir / f"logit_ternario_{experimento['id'].lower()}_metadata.json"
    ...
    metricas = calcular_metricas(y_test, proba_test, classes)
    coeficientes = extrair_coeficientes(modelo)
    feature_names = obter_feature_names(modelo)

    coef_renda = coeficiente_exato(coeficientes, "renda_per_capita", CLASSE_CONTRATADA)
    coef_gap = coeficiente_exato(coeficientes, "gap", CLASSE_CONTRATADA)
    coef_interacao = coeficiente_exato(coeficientes, "renda_gap", CLASSE_CONTRATADA)

    registro = {
        "recorte": recorte,
        "avaliacao": avaliacao,
        "avaliacao_descricao": avaliacao_descricao,
        "modelo": "logit_ternario",
        "modelo_descricao": "Regressão logística multinomial",
        ...
        "target_0": CLASS_LABELS[0],
        "target_1": CLASS_LABELS[1],
        "target_2": CLASS_LABELS[2],
        "roc_auc_ovr_weighted": metricas["roc_auc_ovr_weighted"],
        ...
        "precision_weighted": metricas["precision_weighted"],
        "recall_weighted": metricas["recall_weighted"],
        "f1_weighted": metricas["f1_weighted"],
        "log_loss": metricas["log_loss"],
        "coef_renda_per_capita_classe_contratada": coef_renda,
        "coef_gap_classe_contratada": coef_gap,
        "coef_renda_gap_classe_contratada": coef_interacao,
    ...
    model_metadata = {
        ...
        "observacao": "Modelo logístico multinomial. Métricas calculadas no conjunto indicado por avaliacao_descricao.",
    }
...
def salvar_resultados(registros: list[dict], coeficientes: list[pd.DataFrame], recorte: str, avaliacao: str) -> None:
    ...
    metricas_path = out_dir / "logit_ternario_experimentos_metricas.csv"
    coeficientes_path = out_dir / "logit_ternario_coeficientes.csv"
...
def parse_args_modelagem():
    parser = argparse.ArgumentParser(description="Ajusta regressão logística multinomial.")
