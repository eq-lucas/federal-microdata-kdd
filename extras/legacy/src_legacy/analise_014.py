def orquestrador_inicial_inscritos_14():
...
def orquestrador_ja_rodado_inscritos_14():
    import pandas as pd
    import joblib
    from src.constantes import (
        pasta_data_05_processed_analise_014_X_treino, 
        pasta_data_05_processed_analise_014_y_treino,
        pasta_modelo_analise_014,
        pasta_data_05_processed_analise_014_X_teste,
        pasta_data_05_processed_analise_014_y_teste,
        pasta_data_04_load_inscritos
    )

    X= pd.read_parquet(str(pasta_data_05_processed_analise_014_X_treino))
    Y= pd.read_parquet(str(pasta_data_05_processed_analise_014_y_treino))
    X_teste= pd.read_parquet(str(pasta_data_05_processed_analise_014_X_teste))
    Y_teste= pd.read_parquet(str(pasta_data_05_processed_analise_014_y_teste))
    df_base = pd.read_parquet(str(pasta_data_04_load_inscritos))

    modelo = joblib.load(str(pasta_modelo_analise_014))
...
def gerar_ABT():
    import pandas as pd
    from src.constantes import pasta_data_04_load_inscritos,pasta_data_05_processed_analise_014
  ...
  #  filtro_teste_base= ~(df_base['ano'] == 2019) & ( (df_base['semestre'] == 1) | (df_base['semestre'] == 2) )
    ...
    caminho_salvar = str(pasta_data_05_processed_analise_014)
...
def TreinarModelo(treino_ou_teste_veio: str, x_treino, y_treino):
    ...
    from src.constantes import pasta_modelo_analise_014
    ...
    joblib.dump(pipeline, str(pasta_modelo_analise_014))
    print(f'Modelo salvo em: {str(pasta_modelo_analise_014)}')
...
def prever_probabilidade_treino(
    ...
    modelo = modelo_vindo if usar_modelo == 'simm' \
        else joblib.load(str(pasta_modelo_analise_014))
