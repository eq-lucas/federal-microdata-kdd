def orquestrador_inicial_inscritos_13():
...
def orquestrador_ja_rodado_inscritos_13():
    import pandas as pd
    import joblib
    from src.constantes import (
        pasta_data_05_processed_analise_013_X_treino, 
        pasta_data_05_processed_analise_013_y_treino,
        pasta_modelo_analise_013,
        pasta_data_05_processed_analise_013_X_teste,
        pasta_data_05_processed_analise_013_y_teste,
        pasta_data_04_load_inscritos
    )

    X= pd.read_parquet(str(pasta_data_05_processed_analise_013_X_treino))
    Y= pd.read_parquet(str(pasta_data_05_processed_analise_013_y_treino))
    X_teste= pd.read_parquet(str(pasta_data_05_processed_analise_013_X_teste))
    Y_teste= pd.read_parquet(str(pasta_data_05_processed_analise_013_y_teste))
    df_base = pd.read_parquet(str(pasta_data_04_load_inscritos))

    modelo = joblib.load(str(pasta_modelo_analise_013))
...
def gerar_ABT():
    import pandas as pd
    from src.constantes import pasta_data_04_load_inscritos,pasta_data_05_processed_analise_013
  ...
  #  filtro_teste_base= ~(df_base['ano'] == 2019) & ( (df_base['semestre'] == 1) | (df_base['semestre'] == 2) )
    ...
    filtro_treino_base = (df_base['ano'] == 2019) | ((df_base['ano'] == 2020))

    # Novo Filtro de Teste: 2020-2 em diante
    filtro_teste_base = ((df_base['ano'] == 2021))
    ...
    # Novo Filtro de Treino: Incluindo 2020-1
    filtro_treino = (df_limpo['ano'] == 2019) | ((df_limpo['ano'] == 2020) )

    # Novo Filtro de Teste: 2020-2 em diante
    filtro_teste = ((df_limpo['ano'] == 2021))
    ...
    caminho_salvar = str(pasta_data_05_processed_analise_013)
...
def TreinarModelo(treino_ou_teste_veio: str, x_treino, y_treino):
    ...
    from src.constantes import pasta_modelo_analise_013
    ...
    joblib.dump(pipeline, str(pasta_modelo_analise_013))
    print(f'Modelo salvo em: {str(pasta_modelo_analise_013)}')
...
def prever_probabilidade_treino(
    ...
    modelo = modelo_vindo if usar_modelo == 'simm' \
        else joblib.load(str(pasta_modelo_analise_013))

    chances = modelo.predict_proba(X)[:, 1]

    # GRÁFICO RENDA
    plt.figure(figsize=(12, 6))
    plt.scatter(X['renda_per_capita'], chances, alpha=0.02, s=1)
    ...
    # GRÁFICO GAP
    plt.figure(figsize=(12, 6))
    plt.scatter(X['gap'], chances, alpha=0.02, s=1)
