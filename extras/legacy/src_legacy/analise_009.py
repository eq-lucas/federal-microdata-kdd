def orquestrador_inicial_inscritos_nove():
...
def orquestrador_ja_rodado_inscritos_nove():
    import pandas as pd
    import joblib
    from src.constantes import (
        pasta_data_05_processed_analise_009_X_treino, 
        pasta_data_05_processed_analise_009_y_treino,
        pasta_modelo_analise_009,
        pasta_data_05_processed_analise_009_X_teste,
        pasta_data_05_processed_analise_009_y_teste,
        pasta_data_04_load_inscritos
    )

    X= pd.read_parquet(str(pasta_data_05_processed_analise_009_X_treino))
    Y= pd.read_parquet(str(pasta_data_05_processed_analise_009_y_treino))
    X_teste= pd.read_parquet(str(pasta_data_05_processed_analise_009_X_teste))
    Y_teste= pd.read_parquet(str(pasta_data_05_processed_analise_009_y_teste))
    df_base = pd.read_parquet(str(pasta_data_04_load_inscritos))

    modelo = joblib.load(str(pasta_modelo_analise_009))
...
def gerar_ABT():
    import pandas as pd
    from src.constantes import pasta_data_04_load_inscritos,pasta_data_05_processed_analise_009
  ...
  #  filtro_teste_base= ~(df_base['ano'] == 2019) & ( (df_base['semestre'] == 1) | (df_base['semestre'] == 2) )
    ...
    features_exatas = ['renda_per_capita', 'gap','idade'] + colunas_dummies_opcao_curso + colunas_dummies_conceito_concluiu_curso_superior + colunas_dummies_conceito_curso + colunas_dummies_cine + colunas_dummies_regiao + colunas_dummies_natureza + colunas_dummies_etnia + colunas_dummies_turno + colunas_dummies_ensino_medio_escola_publica
    ...
    caminho_salvar = str(pasta_data_05_processed_analise_009)
...
def TreinarModelo(treino_ou_teste_veio: str, x_treino, y_treino):
    ...
    from src.constantes import pasta_modelo_analise_009
    ...
    joblib.dump(pipeline, str(pasta_modelo_analise_009))
    print(f'Modelo salvo em: {str(pasta_modelo_analise_009)}')
...
def prever_probabilidade_treino(
    ...
    modelo = modelo_vindo if usar_modelo == 'simm' \
        else joblib.load(str(pasta_modelo_analise_009))
...
def acuracia_e_previsao(
    ...
    print(f'''
=============================================================================
CONCLUSÕES DO MODELO PREDITIVO FIES (ELASTICNET 2019-2021)
=============================================================================

1. O FENÔMENO VISUAL DAS "DUAS NUVENS" E A DESISTÊNCIA ECONÔMICA:
A análise visual das predições revelou uma clara segregação dos candidatos em 
duas "nuvens" ou faixas horizontais de probabilidade. Essa separação é a 
materialização matemática das variáveis "opcao_curso_3" (-1.76) e "opcao_curso_2" 
(-1.39), que assumiram as maiores penalidades absolutas do modelo. A nuvem 
inferior concentra os aprovados nessas opções secundárias, evidenciando um forte 
comportamento de evasão: frente à exigência de coparticipação financeira, a massa 
de estudantes desiste no balcão bancário por não identificar custo-benefício em 
assumir uma dívida de longo prazo por um "plano B".

2. O "TETO DE VIDRO" DA RENDA SUPERANDO O MÉRITO ACADÊMICO:
A regularização ElasticNet confirmou que o esforço no ENEM (GAP, +0.22) foi 
rebaixado para a 10ª posição em importância preditiva, enquanto a Renda Per Capita 
(-0.42) se consolidou como o principal "teto de vidro". A dinâmica matemática 
prova que, independentemente da excelência das notas, o aumento da renda 
diminui o percentual de cobertura do governo. A alta exigência de coparticipação 
resultante inviabiliza a contratação, achatando a probabilidade de sucesso e 
tornando o mérito acadêmico uma variável dependente da viabilidade financeira.

3. O "OTIMISMO DO MODELO" (FALSOS POSITIVOS) COMO DESCOBERTA KDD:
O modelo gerou 112.850 Falsos Positivos. Longe de ser um viés algorítmico, esta 
é uma descoberta profunda de mineração de dados: pelas regras do edital educacional 
(nota, área, cota), esses estudantes possuíam o perfil exato para a contratação. 
Contudo, o bloqueio real ocorre na esteira financeira. O modelo previu o sucesso 
acadêmico, mas a realidade impôs o Risco de Crédito (bancos exigindo fiadores 
idôneos de alunos vulneráveis) e a Evasão de Custo. Conclui-se que o FIES opera, 
na prática, como um rigoroso produto de crédito bancário, e não apenas como um 
prêmio de mérito estudantil.

4. O GARGALO DO ALUNO TRABALHADOR E O EFEITO CASCATA DA VULNERABILIDADE:
A interpretabilidade dos coeficientes decodificou a realidade social do programa. 
A penalidade para o turno Noturno (-0.13) reflete um gargalo de concorrência: por 
ser o horário demandado pelo estudante trabalhador, a alta disputa reduz 
drasticamente as chances individuais. Da mesma forma, ser egresso do Ensino Médio 
Público (-0.14) demonstra um efeito cascata de vulnerabilidade, culminando na 
dificuldade de aprovação do crédito bancário. Em contrapartida, mantenedoras 
"Privadas Sem Fins Lucrativos" (+0.24) e "Com Fins Lucrativos" (+0.22) injetam um 
viés positivo, indicando que a estrutura de suporte dessas instituições facilita a 
esteira de contratação mais do que o background social do estudante.

5. DESMISTIFICANDO AS MÉTRICAS: MATRIZ DE CONFUSÃO VS. CLASSES DESBALANCEADAS:
Embora o modelo tenha atingido um excelente ROC-AUC de 0.8441, a base de dados do 
FIES é massivamente desbalanceada (predominância de "Não Contratados"). Por isso, 
a principal métrica de validação adotada foi o Recall na Matriz de Confusão. 
Aplicado sobre os dados cronologicamente futuros (2020-2 a 2021), o modelo atingiu 
um Recall de 90,7% na classe minoritária, identificando corretamente 51.027 dos 
56.255 contratos reais. Isso atesta que a Inteligência Artificial não apenas 
decorou a classe majoritária, mas mapeou com precisão cirúrgica o funil 
determinístico que rege o programa.
=============================================================================
''')
