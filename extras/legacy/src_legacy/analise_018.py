def orquestrador_inicial_inscritos_18():
...
def orquestrador_ja_rodado_inscritos_18():
    import pandas as pd
    import joblib
    from src.constantes import (
        pasta_data_05_processed_analise_018_X_treino, 
        pasta_data_05_processed_analise_018_y_treino,
        pasta_modelo_analise_018,
        pasta_data_05_processed_analise_018_X_teste,
        pasta_data_05_processed_analise_018_y_teste,
        pasta_data_04_load_inscritos
    )

    X= pd.read_parquet(str(pasta_data_05_processed_analise_018_X_treino))
    Y= pd.read_parquet(str(pasta_data_05_processed_analise_018_y_treino))
    X_teste= pd.read_parquet(str(pasta_data_05_processed_analise_018_X_teste))
    Y_teste= pd.read_parquet(str(pasta_data_05_processed_analise_018_y_teste))

    df_base = pd.read_parquet(str(pasta_data_04_load_inscritos))

    modelo = joblib.load(str(pasta_modelo_analise_018))
...
def gerar_ABT():
    import pandas as pd
    from src.constantes import pasta_data_04_load_inscritos,pasta_data_05_processed_analise_018
  ...
  #  filtro_teste_base= ~(df_base['ano'] == 2019) & ( (df_base['semestre'] == 1) | (df_base['semestre'] == 2) )
    ...
    filtro_teste_base = (df_base['subarea_conhecimento'] == 'MEDICINA')

    print('SHAPE de todos os anos', df_base[filtro_treino_base].shape)

    print('SHAPE de todos os anos', df_base[filtro_teste_base].shape)
    ...
    filtro_treino = (df_limpo['subarea_conhecimento_MEDICINA'] == 1)

    # Novo Filtro de Teste: 2020-2 em diante
    filtro_teste = (df_limpo['subarea_conhecimento_MEDICINA'] == 1)
    ...
    caminho_salvar = str(pasta_data_05_processed_analise_018)
...
def TreinarModelo(treino_ou_teste_veio: str, x_treino, y_treino):
    ...
    from src.constantes import pasta_modelo_analise_018
    ...
    joblib.dump(pipeline, str(pasta_modelo_analise_018))
    print(f'Modelo salvo em: {str(pasta_modelo_analise_018)}')
